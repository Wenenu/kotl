...
const LogDetailPage = () => {
    ...
    // File type icons and colors for Important Files
    const FILE_TYPE_CONFIG = {
        'FileZilla': { icon: '📁', color: '#ef4444', description: 'FTP Credentials' },
        'Steam': { icon: '🎮', color: '#1a9fff', description: 'Steam Session' },
        'Outlook': { icon: '📧', color: '#0078d4', description: 'Email Data' },
        'Thunderbird': { icon: '📬', color: '#0a84ff', description: 'Email Client' },
        'WinSCP': { icon: '🔐', color: '#21a366', description: 'SSH/FTP Client' },
        'CiscoVPN': { icon: '🔒', color: '#049fd9', description: 'VPN Config' },
        'OpenVPN': { icon: '🛡️', color: '#ea7e20', description: 'VPN Config' },
        'HeidiSQL': { icon: '🗄️', color: '#4ade80', description: 'Database Client' },
        'DBeaver': { icon: '🐘', color: '#8b5cf6', description: 'Database Client' },
        'VSCode': { icon: '💻', color: '#007acc', description: 'IDE Settings' },
        'Git': { icon: '📝', color: '#f05032', description: 'Git Credentials' },
        'KeePass': { icon: '🔑', color: '#6cae3e', description: 'Password Manager' },
        'Discord': { icon: '💬', color: '#5865f2', description: 'Chat App' },
        'TeamViewer': { icon: '🖥️', color: '#004680', description: 'Remote Access' },
        'RemoteDesktop': { icon: '🖥️', color: '#00a2ed', description: 'RDP Files' },
        'Cyberduck': { icon: '🦆', color: '#ffc107', description: 'FTP Client' },
        '7Zip': { icon: '📦', color: '#000000', description: 'Archive Settings' },
        'Norton': { icon: '🛡️', color: '#ffc40d', description: 'Password Manager' },
        'Origin': { icon: '🎮', color: '#f56c2d', description: 'Gaming Platform' },
        'PrismLauncher': { icon: '⛏️', color: '#3b82f6', description: 'Minecraft Launcher' },
        'MultiMC': { icon: '⛏️', color: '#22c55e', description: 'Minecraft Launcher' },
        'LunarClient': { icon: '🌙', color: '#1e3a8a', description: 'Minecraft Client' },
        'Feather': { icon: '🪶', color: '#8b5cf6', description: 'Minecraft Client' },
        'TLauncher': { icon: '⛏️', color: '#ef4444', description: 'Minecraft Launcher' },
        'Essential': { icon: '✨', color: '#f59e0b', description: 'Minecraft Mod' },
        'ATLauncher': { icon: '⛏️', color: '#0ea5e9', description: 'Minecraft Launcher' },
        '1Password': { icon: '🔐', color: '#0094f5', description: 'Password Manager' },
        'AnyDesk': { icon: '🖥️', color: '#ef443b', description: 'Remote Access' },
        'AutoFTPManager': { icon: '📁', color: '#3b82f6', description: 'FTP Manager' },
        'CloudCredentials': { icon: '☁️', color: '#ff9900', description: 'Cloud Access' },
        'Bitwarden': { icon: '🔐', color: '#175ddc', description: 'Password Manager' },
        'FTPManagerLite': { icon: '📁', color: '#22c55e', description: 'FTP Manager' },
        'FTPRush': { icon: '⚡', color: '#f59e0b', description: 'FTP Client' },
        'GoogleCloud': { icon: '☁️', color: '#4285f4', description: 'Cloud Platform' },
        'NordPass': { icon: '🔐', color: '#4687ff', description: 'Password Manager' },
        'NordVPN': { icon: '🛡️', color: '#4687ff', description: 'VPN' },
        'ProtonVPN': { icon: '🛡️', color: '#6d4aff', description: 'VPN' },
        'RealVNC': { icon: '🖥️', color: '#0052cc', description: 'Remote Access' },
        'SmartFTP': { icon: '📁', color: '#00a2ed', description: 'FTP Client' },
        'TightVNC': { icon: '🖥️', color: '#14b8a6', description: 'Remote Access' },
        'TotalCommander': { icon: '📁', color: '#3b82f6', description: 'File Manager' },
        'UltraVNC': { icon: '🖥️', color: '#6366f1', description: 'Remote Access' },
        'ExodusWallet': { icon: '💎', color: '#8b5cf6', description: 'Crypto Wallet' },
        'RiotGames': { icon: '🎮', color: '#d13639', description: 'Gaming Platform' },
        'SSH': { icon: '🔑', color: '#22c55e', description: 'SSH Keys' },
        'AWS': { icon: '☁️', color: '#ff9900', description: 'AWS Credentials' },
        'SensitiveFile': { icon: '⚠️', color: '#ef4444', description: 'Sensitive Data' },
        'default': { icon: '📄', color: '#94a3b8', description: 'File' }
    };

    const renderImportantFiles = (importantFiles) => {
        if (!importantFiles || importantFiles.length === 0) {
            return <Typography sx={{ p: 2, color: '#94a3b8' }}>No important files found.</Typography>;
        }

        // Filter files based on search query
        const filteredFiles = debouncedSearchQuery 
            ? importantFiles.filter(file => 
                matchesSearch(file.fileName) || 
                matchesSearch(file.fileType) || 
                matchesSearch(file.filePath)
            )
            : importantFiles;

        if (filteredFiles.length === 0 && debouncedSearchQuery) {
            return <Typography sx={{ p: 2, color: '#94a3b8' }}>No important files match your search query.</Typography>;
        }
        ...
        // Sort groups by importance (crypto/password related first)
        const priorityTypes = ['ExodusWallet', 'SSH', 'AWS', 'CloudCredentials', 'KeePass', '1Password', 'Bitwarden', 'NordPass', 'Git', 'SensitiveFile'];
        ...
        return (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                {sortedGroups.map(([fileType, files], groupIndex) => {
                        ...
                        >
                            ...
                            <TableContainer component={Paper} sx={{ backgroundColor: '#1a1f2e', border: '1px solid #334155', borderRadius: '8px' }}>
                                <Table size="small">
                                    <TableHead>
                                        <TableRow>
                                            <TableCell sx={{ color: '#4ade80', fontWeight: 600 }}>File Name</TableCell>
                                            <TableCell sx={{ color: '#4ade80', fontWeight: 600 }}>Path</TableCell>
                                            <TableCell sx={{ color: '#4ade80', fontWeight: 600 }}>Size</TableCell>
                                            <TableCell sx={{ color: '#4ade80', fontWeight: 600 }}>Modified</TableCell>
                                            <TableCell sx={{ color: '#4ade80', fontWeight: 600 }}>Actions</TableCell>
                                        </TableRow>
                                    </TableHead>
                                    <TableBody>
                                        {files.map((file, index) => (
                                            <TableRow key={index} sx={{
                                                backgroundColor: isImportant(file.fileName) || isImportant(file.filePath)
                                                    ? 'rgba(74, 222, 128, 0.1)' 
                                                    : 'inherit',
                                                '&:hover': {
                                                    backgroundColor: 'rgba(255, 255, 255, 0.03)',
                                                ...
                                                <TableCell sx={{ color: '#94a3b8', wordBreak: 'break-all', maxWidth: '300px' }}>
                                                    {file.filePath}
                                                </TableCell>
